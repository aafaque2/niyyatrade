...
// Rules are synced into existing rows on every run — otherwise rule fixes
// never reach databases seeded before the fix.
async function main() {
  const esgFramework = await prisma.framework.upsert({
    where: { slug: "esg" },
    update: { name: "ESG Framework", defaultRules: esgDefaultRules },
    create: {
      slug: "esg",
      name: "ESG Framework",
      defaultRules: esgDefaultRules,
    },
  });

  const aaoifiFramework = await prisma.framework.upsert({
    where: { slug: "halal-aaoifi" },
    update: { name: "AAOIFI", defaultRules: aaoifiDefaultRules },
    create: {
      slug: "halal-aaoifi",
      name: "AAOIFI",
      defaultRules: aaoifiDefaultRules,
    },
  });
  ...
  const bdsFramework = await prisma.framework.upsert({
    where: { slug: "bds" },
    update: { name: "BDS Divestment", defaultRules: bdsDefaultRules },
    create: {
      slug: "bds",
      name: "BDS Divestment",
      defaultRules: bdsDefaultRules,
    },
  });
  ...
  console.log(
    "Seeded frameworks:",
    esgFramework.slug,
    aaoifiFramework.slug,
    standardFramework.slug,
    bdsFramework.slug,
  );
}

const esgDefaultRules = {
  rules: {
    esg_carbon: {
      type: "esg_sector",
      ruleId: "esg_carbon",
    ...
    esg_tobacco_alcohol: {
      ...
      description:
        "Companies whose primary business is the production or sale of tobacco or alcohol. Screened by ticker list because sector-level data cannot distinguish producers from retailers.",
      bannedTickers: [
        // Tobacco
        "MO", "PM", "BTI", "UVV", "VGR", "TPB", "IMBBY", "JAPAY", "JAPAF",
        // Alcohol / breweries / spirits
        "DEO", "STZ", "TAP", "SAM", "BUD", "ABEVF", "HEIAF", "PRMBY",
        "BFAFY", "RBSMY", "ASBRF", "CABGY", "MGPI", "WVVI",
      ],
    },
    esg_employee: {
...
const aaoifiDefaultRules = {
  rules: {
    ...
    debt_to_equity: {
      ...
      threshold: 33.33,
      description:
        "Total Debt cannot exceed 33.33% of the company's market capitalization (AAOIFI Shariah Standard No. 35). Falls back to total assets when market-cap data is unavailable.",
    },
    interest_income: {
      type: "percentage",
